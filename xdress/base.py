...
class XDressPlugin(Plugin):
    ...
    def update_argparser(self, parser):
        ...
        parser.add_argument('--plugins', nargs="+",
                            help=self.rcdocs["plugins"])
        parser.add_argument('--debug', action='store_true',
                            help=self.rcdocs["debug"])
        parser.add_argument('-v', '--verbose', action='store_true',
                            dest='verbose',
                            help=self.rcdocs["verbose"])
        parser.add_argument('--dumpdesc', action='store_true',
                            dest='dumpdesc',
    ...
    def setup(self, rc):
        ...
        if rc.packagedir is NotSpecified:
            rc.packagedir = rc.package.replace('.', sep)
        if not isdir(rc.packagedir):
            os.makedirs(rc.packagedir)
        if not isdir(rc.sourcedir):
            os.makedirs(rc.sourcedir)
        if not isdir(rc.builddir):
            os.makedirs(rc.builddir)
        writenewonly("", join(rc.packagedir, '__init__.py'), rc.verbose)
        writenewonly("", join(rc.packagedir, '__init__.pxd'), rc.verbose)
        rc._cache = DescriptionCache(cachefile=join(rc.builddir, 'desc.cache'))
