...
def _makekey(obj):
    if isinstance(obj, basestring):
        ...
        return tuple([(_makekey(k), _makekey(v))
...
def pycparser_parse(filename, includes=(), defines=('XDRESS',), undefines=(),
    ...
    """
    ext = '.pkl.gz'
    pklgzname = filename.replace(os.path.sep, '_').rsplit('.', 1)[0] + ext
...
def pick_parser(filename, parsers):
    ...
    parsers : str, list, or dict, optional
        ...
        Currently 'clang', 'gccxml', and 'pycparser' are supported, though
        others may be implemented in the future.  If this is a string, then
        this parser is used.  If this is a list, this specifies the parser
        order to use based on availability.  If this is a dictionary, it
        specifies the order to use parser based on language, i.e.
...
class ParserPlugin(Plugin):
    ...
    rcdocs = {
        ...
        'clear_parser_cache_period': ("Number of parser calls to perform"
                                      " before clearing the internal cache. "
                                      "This prevents nasty memory overflow "
                                      "issues."),
    ...
    def execute(self, rc):
        msg = "ParserPlugin is not a complete plugin.  Do not use directly"
        raise TypeError(msg)
